...
namespace EuObjParser
{
	...
	/// Converts Tga to Bmp.
	/// </summary>
	public static class TgaBmpConv
	{
		...
		public static Bitmap Load(Stream stream)
		{
			...
			ushort colorMapOffset = Utilities.LittleEndian(reader.ReadUInt16());
			ushort colorsUsed = Utilities.LittleEndian(reader.ReadUInt16());
			byte bitsPerColorMap = (byte)stream.ReadByte();
			ushort xCoord = Utilities.LittleEndian(reader.ReadUInt16());
			ushort yCoord = Utilities.LittleEndian(reader.ReadUInt16());
			ushort imgWidth = Utilities.LittleEndian(reader.ReadUInt16());
			ushort imgHeight = Utilities.LittleEndian(reader.ReadUInt16());
				...
				if (colorMap > 0)
				{
					...
					if (bitsPerColorMap == 24)
					{
						for (int i = colorMapOffset; i < paletteEntries; i++)
						{
							...
							palette[i] |= (uint)(stream.ReadByte() << 16);
							palette[i] |= (uint)(stream.ReadByte() << 8);
							palette[i] |= (uint)(stream.ReadByte());
						}
					}
					else if (bitsPerColorMap == 32)
					{
						for (int i = colorMapOffset; i < paletteEntries; i++)
						{
							...
							palette[i] |= (uint)(stream.ReadByte() << 8);
							palette[i] |= (uint)(stream.ReadByte());
							palette[i] |= (uint)(stream.ReadByte() << 24);
						}
					}
					else if ((bitsPerColorMap == 15) || (bitsPerColorMap == 16))
					{
						...
						for (int i = colorMapOffset; i < paletteEntries; i++)
						{
							...
							lo = stream.ReadByte();
							palette[i] = 0xFF000000;
							palette[i] |= (uint)((hi & 0x1F) << 3) << 16;
							palette[i] |= (uint)((((lo & 0x3) << 3) + ((hi & 0xE0) >> 5)) << 3) << 8;
							palette[i] |= (uint)(((lo & 0x7F) >> 2) << 3);
						}
					}
				}
